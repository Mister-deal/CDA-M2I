...
-- huitième question
SELECT c.customerNumber, c.customerName, c.contactLastName, c.contactFirstName, c.phone, c.addressLine1, c.city, c.country, p.amount
FROM Customers c
...
SELECT o.orderNumber, o.status, SUM(od.priceEach * od.quantityOrdered) AS totalSales
FROM Orders o
...
WHERE p.productCode = 'S10_1678' AND p.MSRP > od.priceEach;
...
SELECT SUM(amount) AS somme_des_paiements
FROM Payments
WHERE YEAR(paymentDate) = 2005 AND MONTH(paymentDate) = 3;


-- quinzième question

SELECT c.customerName, SUM(p.amount) AS total_des_paiements
FROM Customers c
JOIN Payments p ON c.customerNumber = p.customerNumber
...
SELECT orderDate AS date_de_commande, customerNumber AS numero_client FROM orders where status = "cancelled";

-- dix-septième question

SELECT lastName, firstName FROM employees AS emp WHERE emp.reportsTo IN (
SELECT employeeNumber FROM employees WHERE lastName =  "Bow" AND firstName = "Anthony");

-- dix-huitième question

SELECT lastName, firstName FROM employees AS emp where emp.reportsTo IS NULL;
